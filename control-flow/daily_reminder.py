...
match priority:
    case "high":
        if time_bound == "yes":
            ...
            print(f"Reminder: '{task}' is a high priority task. Try to get to it as soon as possible.")
    case "medium":
        if time_bound == "yes":
    ...
    case "low":
        if time_bound == "yes":
            print(f"Reminder: '{task}' is a low priority task but it's time-sensitive, so do not delay.")
        else:
